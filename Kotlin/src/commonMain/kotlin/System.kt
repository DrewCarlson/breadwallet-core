...
typealias SystemListener = String
typealias BlockchainDb = String
...
interface System {
   ...
   */
  fun configure(appCurrencies: List<BdbCurrency>)
   ...
   */
  fun createWalletManager(network: Network,
                          mode: WalletManagerMode,
                          addressScheme: AddressScheme,
                          currencies: Set<Currency>): Boolean
  ...
  fun subscribe(subscriptionToken: String)
  /**
   ...
   */
  fun updateNetworkFees(completion: CompletionHandler<List<Network>?, NetworkFeeUpdateError?>?)
  ...
  fun getDefaultAddressScheme(network: Network): AddressScheme
  fun getSupportedAddressSchemes(network: Network): List<AddressScheme>
  fun supportsAddressScheme(network: Network, addressScheme: AddressScheme): Boolean
  fun getDefaultWalletManagerMode(network: Network): WalletManagerMode
  fun getSupportedWalletManagerModes(network: Network): List<WalletManagerMode>
  fun supportsWalletManagerMode(network: Network, mode: WalletManagerMode): Boolean
  /**
   * If migration is required, return the currency code; otherwise, return nil.
   *
   * Note: it is not an error not to migrate.
   */
  fun migrateRequired(network: Network?): Boolean
   ...
   */
  fun migrateStorage(network: Network,
  ...
  companion object {
     ...
     */
    fun create(executor: ScheduledExecutorService?,
               listener: SystemListener?,
               account: Account?,
               isMainnet: Boolean,
               storagePath: String?,
               query: BlockchainDb?): System? = TODO()
     ...
     */
    fun asBlockChainDBModelCurrency(uids: String?,
                                    name: String?,
                                    code: String?,
                                    type: String?,
                                    decimals: UInt?): BdbCurrency? = TODO()
     ...
     */
    fun migrateBRCoreKeyCiphertext(
        key: Key?,
        nonce12: ByteArray?,
        authenticatedData: ByteArray?,
        ciphertext: ByteArray?): ByteArray? = TODO()
     ...
     * returns.
     */
    fun wipe(system: System?): Unit = TODO()
     ...
     */
    fun wipeAll(storagePath: String, exemptSystems: List<System>): Unit = TODO()
  }
}
